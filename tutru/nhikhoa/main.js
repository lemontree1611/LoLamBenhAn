...
`.trim();
  ...
  try {
        const response = await fetch("https://lolambenhan-0be9.onrender.com/chat", {
      method: "POST",
...
function updateTomtat() {
  ...
  const ready = !!gioitinh && !!tuoiText && !!lydoText;
  if (!ready) {
...
<body>
  ...
  <p><b>1. Toàn trạng:</b><br/>
    ...
    ${formatSmartPreviewLine(`- Chiều cao: ${data.chieucao} cm, cân nặng: ${data.cannang} kg, Đánh giá Z-score: ${data.wa}; ${data.ha}; ${data.wh}`)}<br/>
...
(function initShareWebSocket() {
  ...
  function setFieldLockedUI(fieldId, locked, byWho) {
    ...
    if (locked) {
      ...
      // input type="date"/select có thể không hiện placeholder, nhưng vẫn set để thống nhất
      try { el.placeholder = `Đang được sửa bởi ${who}`; } catch (_) {}
    } else {
      el.disabled = false;
      ...
      const old = el.getAttribute("data-old-placeholder");
      if (old !== null) {
        try { el.placeholder = old; } catch (_) {}
      }
      el.removeAttribute("data-old-placeholder");
    }
  }
  ...
  function applyLocks(locksObj) {
    ...
    // unlock tất cả lock cũ (không phải của mình) trước khi apply list mới
    for (const [fid, meta] of Object.entries(state.locks || {})) {
      if (meta?.by && meta.by !== state.clientId) setFieldLockedUI(fid, false);
    }
...
function snapshotData() {
  ...
  function __resetFormUIOnly() {
    ...
    // Ẩn khu đánh giá Z-score + clear status nếu có
    try { showZscoreSection(false); } catch (_) {}
    try { setZscoreStatus(""); } catch (_) {}
