...
function updateTomtat() {
  ...
  const text = `Sản phụ ${tuoi} tuổi, PARA ${para}, vào viện vì lý do ${lowercaseFirstText(lydo)}, dự sinh ${formatDDMMYYYY(due)}. Qua hỏi bệnh, khám bệnh ghi nhận:`;
...
(function initShareWebSocket() {
  ...
  function applyData(dataObj) {
    ...
    try { if (window.__SHARE_SYNC__) window.__SHARE_SYNC__.applyingRemote = true; } catch (_) {}
    try {
      ...
      try { if (window.__SHARE_SYNC__) window.__SHARE_SYNC__.applyingRemote = false; } catch (_) {}
    }
  }
