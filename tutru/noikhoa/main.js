...
if (bienLuanBtn) {
  bienLuanBtn.addEventListener("click", async () => {
    ...
    if (!mainDiag && !differential) {
      alert("Hãy nhập chẩn đoán chính hoặc chẩn đoán phân biệt trước.");
      return;
    }
    ...
    try {
      ...
      const systemPrompt = `
Bạn là bác sĩ hỗ trợ viết phần biện luận bệnh án.
      ...
      const response = await fetch(CHAT_API_URL, {
        ...
        body: JSON.stringify({
          messages: [
            ...
            { role: "user", content: `Dữ kiện bệnh án:\n${text}` }
          ]
        })
      });
