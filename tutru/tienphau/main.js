...
function _buildBienLuanContext() {
  ...
  const parts = [
    ...
    _getBienLuanField("benhngoai", "Bệnh ngoại khoa"),
...
(function initShareWebSocket() {
  ...
  function snapshotData() {
    ...
    // Backward-compat when another client still uses old id `cls_chuandoan`.
    if (typeof out.cls_canlamsang === "string" && !("cls_chuandoan" in out)) {
      out.cls_chuandoan = out.cls_canlamsang;
    }
    return out;
  }
  ...
  function applyData(dataObj) {
    ...
    try {
      for (const el of collectFields()) {
        ...
        let v = dataObj[el.id];
        // Backward-compat for old shared payloads that still send `cls_chuandoan`.
        if (el.id === "cls_canlamsang" && v === undefined) {
          v = dataObj.cls_chuandoan;
        }
